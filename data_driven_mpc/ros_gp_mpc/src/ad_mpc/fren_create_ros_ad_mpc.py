...
from ad_mpc.fren_ad_3d_mpc import Fren_AD3DMPC
import numpy as np
import std_msgs.msg
import rospy
import os
...
def custom_ad_param_loader(ad_name):
    ...
    ad = Fren_AD3D(noisy=False, noisy_input= False)
...
class Fren_ROSGPMPC:
    def __init__(self, t_horizon, n_mpc_nodes, opt_dt, ad_name = "sim_car", point_reference=False):
        ...
        q_diagonal = np.array([0.0, 10.0, 10.0, 100.0, 0.1])
    ...
    def set_reference(self, x_ref, u_ref, curv_ref,cdist_ref, terminal_point = False):
        """
        ...
        """
        return self.ad_mpc.set_reference(x_reference=x_ref, curv_ref = curv_ref, cdist_ref = cdist_ref,u_reference=u_ref, terminal_point = terminal_point)
    ...
    def optimize(self, model_data):
        ...
        next_control_with_stamp.drive.steering_angle = x_opt[0,4]
